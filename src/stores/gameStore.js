  ...
  actions: {
    ...
    endTurn() {
      ...
      if (this._loanBalance > 0) {
        ...
        if (remaining === 1) this.addLog(`?? [페잔] 다음 턴이 차관 상환 기일입니다. (${this._loanBalance.toLocaleString()} 마크)`)
        if (remaining <= 0) {
          // 상환 불능 처리
        ...
        switch (opType) {
          ...
          case 'ASSASSIN': {
            ...
            if (enemyFleets.length > 0) {
              ...
              this.addLog(`??? [암살] ${sys.name}의 ${CHARACTERS?.[target.commander]?.name || target.commander} 제거 성공!`)
              target.commander = null
            } else {
              this.addLog(`??? [암살] ${sys.name}에서 요인을 제거했습니다.`)
            }
            break
          }
        }
      ...
      ['REH','FPA','PZN'].filter(f => f !== this.playerFaction).forEach(f => {
        ...
        ;(this.fleets[f] || []).forEach(fleet => {
          ...
          if (Math.random() < 0.5) {
            ...
            t.faction = f
            this.addLog(`?? [AI] ${FACTIONS[f].name} ${fleet.name}이 ${t.name} 점령! (${prev || '중립'} → ${f})`)
          }
        })
      })
    },
