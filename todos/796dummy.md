...
라인하르트 총사령관 휘하 약 2만척. [2026-07-04] 로엔그람 함대(REH004) 위치를 아스타테(230005)로 확정, `buildFleetsMap()`이 `parentFlt` 분함대(REH041~045) 함선 수를 상위 함대에 합산하도록 수정 완료 — 이제 게임 내 REH 총 병력이 24,000척으로 정상 반영됨(`gap_analysis.md` 항목 15 참조).

| 함대 코드 | 함대명 | 사령관 | 부관/참모 | 함선 수 | 시작 위치 |
|---|---|---|---|---|---|
| `REH001` | 뮈켄베르거 함대 | 뮈켄베르거 (CH_000199) | — | 4,000 | ⚠ `fleetData.js`에 데이터 없음 (문서만 존재, 별도 TODO) |
| `REH004` | 로엔그람 함대 (총사령관) | 라인하르트 폰 뮈젤 (CH_000064) | 키르히아이스 (CH_000388) | 4,000 | 230005 (아스타테) |
| `REH041` | 메르카츠 분함대 | 메르카츠 (CH_000195) | — | 4,000 | parentFlt: REH004 (함선 수 REH004에 합산) |
| `REH042` | 슈타덴 분함대 | 슈타덴 (CH_000223) | — | 4,000 | parentFlt: REH004 (함선 수 REH004에 합산) |
| `REH043` | 파렌하이트 분함대 | 파렌하이트 (CH_000233) | — | 4,000 | parentFlt: REH004 (함선 수 REH004에 합산) |
| `REH044` | 에를라흐 분함대 | 에를라흐 (CH_000290) | — | 4,000 | parentFlt: REH004 (함선 수 REH004에 합산) |
| `REH045` | 포겔 분함대 | 포겔 (CH_000494) | — | 4,000 | parentFlt: REH004 (함선 수 REH004에 합산) |

> ⚠ 분함대 사령관(메르카츠/슈타덴/파렌하이트/에를라흐/포겔)은 함선 수만 REH004에 합산될 뿐, `officers` 목록에는 아직 반영되지 않음 (REH004의 `charList`는 라인하르트/키르히아이스만 존재) — 별도 TODO.
>
> 이제르론 주둔함대
...
- [x] REH 함대 초기 locCode 확정 (아스타테 230005로 확정, 2026-07-04) — `buildFleetsMap()` 분함대 합산 로직도 함께 수정
